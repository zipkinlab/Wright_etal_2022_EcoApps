...
## Code: Data simulation and analysis for the Weighted Effort case study
...
## Looping Variables
start <- 1
end <- 10 #Run this script 65 times in HPCC for a total of 650 sims: for i in {1..65}; do sbatch amphibianWE.sb; done
...
for(i in start:end){ 
  ...
  strategy <- "weightEff"
  effort <- 0.1
  Jr_temp[[i]] <- round(td$Jr*effort) 
  maxJr_temp[[i]] <- max(Jr_temp[[i]])
  K_temp[[i]] <- c(6,6,2,2,6,6,2,2,6,2)  #Parks of 6: c(1,2,5,6,9) ; Parks of 2: c(3,4,7,8,10)  
